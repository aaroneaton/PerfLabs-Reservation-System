...
class CAS_auth_model extends CI_Model
{
	/**
	...
	public function __construct()
	{
		parent::__construct();
		$this->load->database();
		$this->load->config('cas_auth', TRUE);
		$this->load->helper('cookie');
		$this->load->helper('date');
		$this->load->library('session');

		$this->tables  = $this->config->item('tables', 'cas_auth');
		$this->columns = $this->config->item('columns', 'cas_auth');

		$this->identity_column     = $this->config->item('identity', 'cas_auth');
		$this->store_salt      	   = $this->config->item('store_salt', 'cas_auth');
		$this->salt_length     	   = $this->config->item('salt_length', 'cas_auth');
		$this->meta_join       	   = $this->config->item('join', 'cas_auth');
		$this->min_password_length = $this->config->item('min_password_length', 'cas_auth');
	...
	public function username_check($username = '')
	{
	    ...
	    return $this->db->where('username', $username)
			    ->where($this->cas_auth->_extra_where)
...
/*
		if (isset($this->cas_auth->_extra_where) && !empty($this->cas_auth->_extra_where))
	    {
			$this->db->where($this->cas_auth->_extra_where);
	    }
*/
	...
	public function register($username, $password, $email, $additional_data = false, $group_name = false)
	{
	    ...
	    {
		$this->cas_auth->set_error('account_creation_duplicate_email');
		return FALSE;
	    }
	    elseif ($this->identity_column == 'username' && $this->username_check($username))
	    {
		$this->cas_auth->set_error('account_creation_duplicate_username');
		return FALSE;
	    }
		...
		if(!$group_name)
		{
		    $group_name = $this->config->item('default_group', 'cas_auth');
		}
	    ...
	    if($this->cas_auth->_extra_set)
	    {
		$this->db->set($this->cas_auth->_extra_set);
	    }
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		if ($result->password === $password)
		{
		    ...
		    if ($remember && $this->config->item('remember_users', 'cas_auth'))
		    {
	...
	public function update_user($id, $data)
	{
	    ...
	    if (array_key_exists($this->identity_column, $data) && $this->identity_check($data[$this->identity_column]) && $user->{$this->identity_column} !== $data[$this->identity_column])
	    {
			...
			$this->cas_auth->set_error('account_creation_duplicate_'.$this->identity_column);
			return FALSE;
	    }
	...
	public function set_lang($lang = 'en')
	{
	    set_cookie(array(
			'name'   => 'lang_code',
			'value'  => $lang,
			'expire' => $this->config->item('user_expire', 'cas_auth') + time()
			    ));
	...
	public function login_remembered_user()
	{
		...
		if ($this->config->item('user_extend_on_login', 'cas_auth'))
		{
	...
	private function remember_user($id)
	{
	    ...
	    if ($this->db->affected_rows() > -1)
	    {
			set_cookie(array(
					...
					'expire' => $this->config->item('user_expire', 'cas_auth'),
					));
